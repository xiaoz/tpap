KISSY.add('init-countdown-widgets', function(S){
    var DOM = S.DOM,
        timeUnits = {
...
KISSY.use('switchable, init-countdown-widgets', function(S, Switchable, Countdown){
    var DOM = S.DOM;
    var cls = 'J_TScriptedModule';

    function initWidget(hook, container) {
        hook = '.' + (hook || 'KS_Widget');
        var wds = {
            ...
            widgetElems = S.query(hook, container);

        eachElem();
        function eachElem() {
            S.later(function () {
                if (widgetElems.length !== 0) {//数组还有模块要进行初始化
                    try {
                        initEachWidget(widgetElems.shift()); //从数组中取出一个模块并进行初始化
                    } catch (err) {
                        S.log("初始化组件出错");
                    }
                    eachElem();
    ...
    KISSY.each(DOM.query('.' + cls), function (container) {
        initWidget("J_TWidget", container);
    });
});
