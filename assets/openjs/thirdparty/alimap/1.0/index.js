    ...
    function init(frameGroup) {
        ...
        //--------------------------------------
            ...
            SafeAliMarker.prototype.getLatLng = function () {
                return cajaAFTB.untame(this.obj.getLatLng())      //todo 是否正确
            };
            ...
            function SafeAliEvent() {
                this.obj = AliEvent;
            }

            funs = ('addListener').split(',');
            frameGroup.markCtor(SafeAliEvent);

            SafeAliEvent.prototype.addListener = function () {
                var param = arguments;
                this.obj.addListener(param[0].obj,param[1], frameGroup.markFunction(function(e){
                    var point = {
                        x: e.x,
                        y: e.y,
                        la: e.la,
                    ...
                    param[2].call(this, frameGroup.tame(point));
                }));
            };

            S.each(funs, function (fn) {
                frameGroup.grantMethod(SafeAliEvent, fn);
            });
;
        ...
        return function (context) {
            ...
            return {
                ...
                AliEvent: frameGroup.tame(frameGroup.markFunction(function () {
                    return new SafeAliEvent()
                })),
                kissy: false
            }
        }
